...
// --- [MODIFIED] Mongoose Connection for Serverless ---
let conn = null;
...
//     const { mainTopic, toughness, readingMaterial, learningOutcomes } = req.body;
//     try {
//         if (!mainTopic || !readingMaterial || !learningOutcomes || learningOutcomes.length === 0) {
//            return res.status(400).json({ error: "Missing required parameters." });
//         }
//         const genAI = new GoogleGenerativeAI(process.env.API_KEY);
//         const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
...
//             ---
//             **Rules for 'CODE_ANALYSIS_MULTIPLE_CHOICE':**
//             1.  **Question Style:** Present a code snippet and ask to predict the output, find an error, or identify an error type.
//             2.  **Options:** Provide multiple options, with only one being correct. Each option object MUST have an "is_correct" boolean field.

//             **Rules for 'CODE_ANALYSIS_TEXTUAL' (Fill-in-the-Blank):**
//             1.  **Question Style:** Present a code snippet that includes a placeholder like \`_________\`. The question should ask what to fill in the blank to achieve a specific outcome.
//             2.  **Options:** The "options" array MUST contain exactly ONE object, which is the correct text to fill in the blank. It must have \`"is_correct": true\`.
//             ---

//             **General Rules for ALL Questions:**
//             1.  **Self-Contained:** Questions must be understandable without external context. DO NOT use phrases like "according to the text".
//             2.  **Bloom's Level:** Each question MUST have a "bloom_level" field (APPLYING, ANALYZING, etc.).
//             3.  **Content Separation:** The "question" object must have "content" (text) and "code_snippet".
//             4.  **Core Concept:** Each question object must have a "core_concept".
//             5.  **Single Skill:** The "skills" array MUST contain exactly one string, which is the specific learning outcome being assessed.
...
//             \`\`\`json
//             { "mcqs": [ { "core_concept": "string", "question_key": "string", "bloom_level": "string", "skills": ["string (MUST contain only ONE most relevant skill)"], "toughness": "${toughness.toUpperCase()}", "question_type": "string (Either 'CODE_ANALYSIS_MULTIPLE_CHOICE' or 'CODE_ANALYSIS_TEXTUAL')", "question": { "content": "string", "code_snippet": "string", "content_type": "MARKDOWN" }, "options": [ { "content": "string", "is_correct": true } /* more options for multiple choice */ ], "explanation_for_answer": { "content": "string" } } ] }
//             \`\`\`
//             Reading Material to Use:
//             ---
//             ${readingMaterial}
